...
# Additional initialization for opensource version
if "embedding_model" not in st.session_state:  # for opensource version
    st.session_state["embedding_model"] = "nomic-embed-text"

def handle_embedding_model_change():
    """Clean up everything when embedding model or LLM model changes."""
    ...
    if "graph_index" in st.session_state:
        st.session_state["graph_index"] = None
        st.warning("Knowledge graph index cleared due to model change.")
    
    # Clear chat history as it might contain references to old models
    if "chat_history" in st.session_state:
        st.session_state["chat_history"] = []
        st.info("Chat history cleared due to model change.")
    
    # Force reprocessing of documents
    st.warning("Please reprocess your documents with the new model settings.")
...
with st.sidebar:
    ...
    st.markdown(""" 
                <h2 style='color: red;'><strong>Step 2: Ollama Model Configuration</strong></h3>""", unsafe_allow_html=True)

    st.session_state["model_name"] = st.selectbox(
        "**Select Ollama model:**",
        ["mistral", "llama3.1:8b"],
        index=[ "mistral", "llama3.1:8b"].index(st.session_state["model_name"]),
        key="model_name_select"
    )

    st.markdown("""
    <h3 style='color: red;'><strong>Embedding Model Selection</strong></h3>
    """, unsafe_allow_html=True)
    st.session_state["embedding_model"] = st.selectbox(
    "**Select Embedding Model:**",
    ["nomic-embed-text", "jina/jina-embeddings-v2-base-en"],
    index=["nomic-embed-text", "jina/jina-embeddings-v2-base-en"].index(st.session_state["embedding_model"]),
...
def handle_library_id_change():
    ...
    if "previous_library_id" in st.session_state:
        if library_id != st.session_state["previous_library_id"]:
            ...
            # Clean up graph index and visualization
            if "graph_index" in st.session_state:
                try:
...
def get_embeddings_model(embedding_model):
    """Get the appropriate embeddings model based on selection."""
    try:
        if embedding_model == "nomic-embed-text":
            return OllamaEmbeddings(model="nomic-embed-text")
        elif embedding_model == "jina/jina-embeddings-v2-base-en":
            return OllamaEmbeddings(model="jina/jina-embeddings-v2-base-en")
        else:
...
def initialize_embeddings():
    ...
    try:
        st.session_state["current_embeddings"] = get_embeddings_model(st.session_state["embedding_model"])
        return True
    except Exception as e:
...
def process_with_semantic_chunking(pdf_paths):
    """Process PDFs using semantic chunking method."""
    try:
        split_docs = []
        
        with st.spinner("Processing documents with semantic chunking..."):
            # Use the embeddings from session state
            if st.session_state["current_embeddings"] is None:
                if not initialize_embeddings():
                    return None, None, "Failed to initialize embeddings"
            
            # Initialize the semantic chunker with the shared embeddings
            ...
            if split_docs:
                ids = [str(i) for i in range(1, len(split_docs) + 1)]
                return split_docs, ids, f"{len(split_docs)} chunks were successfully created using semantic chunking with {st.session_state['embedding_model']}"
            else:
                return None, None, "No chunks were created."
...
def create_graph_index(split_docs):
    ...
    try:
        ...
        llm = Ollama(model= st.session_state["model_name"] , temperature=0)
        Settings.llm = llm
        ...
        return None, f"Error creating knowledge graph index: {str(e)}"
...
def answer_question(question, graph_index):
    ...
    try:
        ...
        if "truth value of an array" in str(e):
            # Retry once if we get the array truth value error
            time.sleep(2)
            return answer_question(question, graph_index)
        return {
            "answer": f"Error generating response: {str(e)}",
            "sources": []
        }    
...
# Modify the document processing section to immediately create and display the graph
if process_button and "pdf_paths" in st.session_state:
    ...
    if split_docs:
        ...
        with st.spinner("Creating knowledge graph index... This may take a few minutes."):
            graph_index, graph_message = create_graph_index(split_docs)
...
#         question = st.text_input("Question", key="question_input", label_visibility="collapsed", placeholder="Enter your question here...")
