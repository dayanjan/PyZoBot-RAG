...
def get_embeddings_model(model_name):
    ...
    try:
        ...
        elif model_name == "jina/jina-embeddings-v2-base-en":
...
def handle_embedding_model_change():
    """Clean up everything when embedding model changes."""
    ...
    if "db" in st.session_state:
        ...
        # Remove the database from session state
        del st.session_state["db"]
    
    # Clear chat history as it might contain references to old embeddings
    if "chat_history" in st.session_state:
        st.session_state["chat_history"] = []
    # Force reprocessing of documents
    st.warning("Please reprocess your documents with the new embedding model.")
        ...
        with st.spinner("Processing documents with semantic chunking..."):
            ...
            for pdf_path in pdf_paths:
                try:
                    ...
                    # st.success(f"Successfully processed: {source}")
...
def create_vector_store(split_docs, ids, library_id):
    ...
    try:
        ...
        with st.spinner("Creating embeddings and building vector store..."):
            # Always initialize new embeddings
            if not initialize_embeddings():
                return None, None, "Failed to initialize embeddings"
            ...
            if "chroma_persist_dir" in st.session_state:
                try:
                    ...
                    st.error(f"Error cleaning up old directory: {str(e)}")
            ...
            # Create new database with clean embeddings
            ...
            return db, persist_directory, f"Vector store successfully created using {st.session_state['embedding_model']} embeddings."
...
if "pdf_paths" in st.session_state:
    if process_button:
        try:
            ollama.list()  # Quick check if Ollama is running
            if "db" in st.session_state:
                if "chroma_persist_dir" in st.session_state:
                    shutil.rmtree(st.session_state["chroma_persist_dir"], ignore_errors=True)
                del st.session_state["db"]
            
            # Reset embeddings when processing starts
            st.session_state["current_embeddings"] = None
            ...
            st.error(f"Error: Make sure Ollama is running. Details: {str(e)}")
...
def answer_question(question, db):
    ...
    compression_retriever = ContextualCompressionRetriever(
        ...
        base_retriever=db.as_retriever(
            search_type="mmr", 
            search_kwargs={"k": st.session_state["k"], "lambda_mult": 0.25}
        ),
    )
